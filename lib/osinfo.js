    ...
    process.nextTick(() => {
      ...
      if (_windows) {
        ...
        try {
          ...
          workload.push(util.powerShell('reg query "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\CapabilityAccessManager\\ConsentStore\\location" /v Value'));
          util.promiseAll(workload).then((data) => {
            ...
            if (data.results[5]) {
              const raw = data.results[5].toString();
              const match = raw.match(/^\s*Value\s+REG_SZ\s+([^\r\n]+)/im);
              result.locationservice = match ? match[1].trim().toLowerCase() : '';
            }
